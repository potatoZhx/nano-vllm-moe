...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--plot_type", choices=["heatmap", "lines"], default="heatmap", help="One-figure visualization style.")
...
def plot_lines(plot_df: pd.DataFrame, args: argparse.Namespace) -> None:
    ...
    for condition, sub in plot_df.groupby("condition", sort=False):
        sub = sub.sort_values("layer_idx")
        ax.plot(sub["layer_idx"], sub[value_col], marker="o", markersize=3, linewidth=1.4, label=condition)
    ...
    ax.set_xticks(tick_positions)
    ax.legend(title="Condition", ncols=2, fontsize=8)
    fig.tight_layout()
    fig.savefig(args.out, dpi=args.dpi, bbox_inches="tight")
    plt.close(fig)
