...
def heterogeneous_moe_forward(
    ...
    if plan.gpu_token_count > 0:
        gpu_hidden = expanded_hidden[plan.gpu_mask]
        gpu_hidden = gpu_hidden[plan.sort_idx]
        ...
        gpu_expert_out = gpu_expert_out[plan.inv_sort_idx]

        gpu_token_indices = token_indices[plan.gpu_mask]
        gpu_weights = flat_weights[plan.gpu_mask]
    ...
    if plan.cpu_token_count > 0:
        ...
        cpu_hidden = expanded_hidden[cpu_mask]
        cpu_experts = flat_selected[cpu_mask]
        cpu_token_indices = token_indices[cpu_mask]
        cpu_weights = flat_weights[cpu_mask]
